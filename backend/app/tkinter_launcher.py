...
def _resolve_model_dir() -> Path:
    if getattr(sys, "frozen", False):
        ...
        if bundled.is_dir() and ((bundled / MODEL_CHECK_FILE).exists() or (bundled / MODEL_CHECK_FILE_SAFETENSORS).exists()):
            return bundled
...
def _check_model_downloaded(model_dir: Path) -> bool:
    return model_dir.is_dir() and ((model_dir / MODEL_CHECK_FILE).exists() or (model_dir / MODEL_CHECK_FILE_SAFETENSORS).exists())
...
class _LauncherWindow:
    ...
    def _build_ui(self):
        ...
        # Title
        tk.Label(
            self.root, text="AuditBee 启动器", font=title_font,
            bg=BG, fg=PRIMARY,
        ).pack(pady=(20, 5))
        tk.Label(
            self.root, text="选择大模型供应商并检查嵌入模型", font=small_font,
            bg=BG, fg="#6B7280",
        ).pack(pady=(0, 15))

        # --- LLM Provider Section ---
        tk.Label(
            self.root, text="大模型配置", font=section_font, bg=BG, fg=FG,
        ).pack(**pad, anchor="w")
        ...
        provider_combo = ttk.Combobox(
            row_provider, textvariable=self.provider_var,
            values=PROVIDER_LABELS, state="readonly", width=28,
        )
        provider_combo.pack(side="left", padx=(5, 0))

        tk.Label(
            self.root, text="API Key 和模型参数请在启动后进入「设置」页面配置",
            font=small_font, bg=BG, fg="#6B7280",
        ).pack(padx=20, anchor="w", pady=(0, 5))

        # --- Model Download Section ---
        tk.Label(
            self.root, text="嵌入模型（可选）", font=section_font, bg=BG, fg=FG,
        ...
        tk.Label(
            self.root,
            text=f"模型: {MODEL_ID}\n大小: ~1.3 GB（知识图谱功能需要）",
            font=small_font, bg=BG, fg="#6B7280", justify="left",
        ...
        self.model_status_label = tk.Label(
            self.root, textvariable=self.model_status_var,
            font=small_font, bg=BG, fg=SUCCESS if self._model_downloaded else "#6B7280",
        )
        ...
        if not self._model_bundled:
            ...
            self.download_btn = tk.Button(
                row_model, text="下载模型", font=normal_font,
                command=self._on_download_model, relief="flat",
                bg="#E8E5E0", fg=FG, cursor="hand2",
            )
            self.download_btn.pack(side="left", padx=(5, 10))

            self.skip_btn = tk.Button(
                row_model, text="跳过", font=small_font,
                command=self._on_skip_model, relief="flat",
                bg=BG, fg="#6B7280", cursor="hand2",
            )
        ...
        self.start_btn = tk.Button(
            self.root, text="启动 AuditBee", font=tkfont.Font(family="Microsoft YaHei", size=12, weight="bold"),
            bg=PRIMARY, fg="white", relief="flat", cursor="hand2",
            command=self._on_start, width=25, height=2,
        )
        self.start_btn.pack(pady=(10, 5))

        # Footer hint
        tk.Label(
            self.root, text="启动后请在「设置」页面配置 API Key",
            font=small_font, bg=BG, fg="#6B7280",
        ).pack(pady=(0, 15))
...
def show_launcher() -> Optional[Dict[str, str]]:
