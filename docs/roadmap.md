...
| P0-2 | ~~审批操作重新执行整个 agent 链路~~ | ~~2x LLM 成本，结果可能不一致~~ | 已修复: approve 直接完成任务，不重跑管线 (v1.1) |
...
| P0-6 | ~~单文档处理串行执行~~ | ~~多文档审计耗时线性增长~~ | 已支持多文档并行处理 + chunk 并行分析 (v1.1) |
...
| P1-2 | ~~审计结果缓存~~ | ~~相同文档+相同配置的审计结果缓存，避免重复 LLM 调用~~ | Done: per-document 内存缓存，TTL 30 分钟 |
| P1-3 | ~~审批不重跑链路~~ | ~~approve 操作直接完成任务，不重新执行 agent 链路~~ | Done |
| P1-4 | ~~文档处理失败重试~~ | ~~添加手动重新处理按钮 + 自动重试机制~~ | Done: 重试按钮 + POST /documents/{id}/process |

### 1.2 Human-in-the-Loop 增强

| 编号 | 改进项 | 描述 | 预期效果 |
|------|--------|------|----------|
| P1-5 | ~~Finding 级别审批~~ | ~~支持逐条 findings 审批/拒绝/修改~~ | Done: FindingStatus + approve/reject 端点 + 前端按钮 |
| P1-6 | ~~审批批注~~ | ~~审批时添加批注说明~~ | Done: task 和 finding 级别均支持 comment |
| P1-7 | 审计前确认 | 触发审计前显示预估 LLM 调用次数和成本 | 后端已完成，前端集成中 |
...
| P1-10 | ~~Per-LLM-call timeout~~ | ~~每个 LLM 调用独立超时（90s），而非全局 300s~~ | Done: asyncio.wait_for + 90s timeout |
| P1-11 | ~~LightRAG LLM 调用重试~~ | ~~为 LightRAG 的 httpx 调用添加重试机制~~ | Done: 3 次重试 + 指数退避 |
| P1-12 | ~~启动器模型名称配置~~ | ~~启动动器支持自定义模型名称，无需进入设置页~~ | Done |
| P1-13 | ~~设置批量保存优化~~ | ~~.env 文件批量写入，避免逐 key 重写~~ | Done |
| P1-14 | ~~文件夹扫描 / 自动发现~~ | ~~启动时自动注册未入库文件~~ | Done: startup 扫描 data/documents/ |
| P1-15 | ~~审计记忆积累~~ | ~~将审计发现持久化为 JSONL~~ | Done: memory.py + JSONL + /api/audit/memory |
